...
## appears under the finger only once it moves, so a plain tap in the region
## still reaches gameplay. Named TouchJoystick because Godot 4.7 ships its own
...
@export var show_zone_hint: bool = false
...
func _draw() -> void:
	if show_zone_hint and not active:
		var z := _zone_px().grow(-14.0)
		draw_rect(z, Color(1, 1, 1, 0.05), true)
		draw_rect(z, Color(1, 1, 1, 0.3), false, 2.0)
		draw_circle(z.get_center(), 6.0, Color(1, 1, 1, 0.3))
	if not active:
		return
